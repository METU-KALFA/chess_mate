...
class camera_XYZ:
    ...
    def calculate_xyz(self, pixel, new_depth_frame, new_frame):
        global depth_intrin, color_intrin, depth_to_color_extrin, color_to_depth_extrin

        print("calculate xyz e girdi")
        ...
        if intrinsics:
            depth_point_ = rs2.rs2_project_color_pixel_to_depth_pixel(
                depth_frame.get_data(), depth_scale,
                depth_min, depth_max,
                depth_intrin, color_intrin, depth_to_color_extrin, color_to_depth_extrin, pixel)
            print("depth_point:", depth_point_)
        return depth_point_
...
def capture_and_detect():
    ...
    if new_frame is not None:
        ...
        # Uncomment the lines with imshow to see the detected board centers
    ...
    final_xyz_dict = camera.convert_to_xyz_v2(final_pixel_dict, new_depth_frame, new_frame)
    ...
    if req.qry == "get_capture_piece_coord":
        ...
        for piece in captured_pieces:
            if piece[0] == piece_name:
                ...
                captured_queen_coords = (0.5093303, -0.31486746, 0.16721719283)
...
def listener():
    ...
    config = rs2.config()
    config.enable_stream(rs2.stream.depth, 640, 640, rs2.format.z16, 10)
    config.enable_stream(rs2.stream.color, 640, 640, rs2.format.rgb8, 10)

    # Start the pipeline
    pipeline.start(config)

    # Get the stream profiles
    depth_stream_profile = pipeline.get_active_profile().get_stream(rs2.stream.depth)
    color_stream_profile = pipeline.get_active_profile().get_stream(rs2.stream.color)

    # Get intrinsics and extrinsics
    depth_intrin = depth_stream_profile.as_video_stream_profile().get_intrinsics()
    color_intrin = color_stream_profile.as_video_stream_profile().get_intrinsics()
    depth_to_color_extrin = depth_stream_profile.get_extrinsics_to(color_stream_profile)
    color_to_depth_extrin = color_stream_profile.get_extrinsics_to(depth_stream_profile)
