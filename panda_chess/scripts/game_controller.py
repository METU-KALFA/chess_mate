...
def chess_table_to_fen(chess_table, set_init_fen):
    ...
    # TODO UNCOMMENT THIS PART AFTER PERFECTING THE MODEL (THIS FUNCTION CHECKS IF THE GIVEN FEN STRING IS VALID CONSIDERING CHESS RULES)
    # FOR EXAMPLE IT WILL NOT ALLOW THE GAME TO HAVE MORE THAN 2 KINGS
...
def find_move_between_fen(board_before, fen_after):
    ...
    print("Board before player move")
    print(board_before)
...
# Allows the player to make a move using the terminal due to model detection inconsistencies
def make_move_manually():
...
def update_board(data):
    ...
    if move == -1:
        ...
        print("Or enter the player move here")
        new_move = make_move_manually()
        ...
        if stockfish.is_move_correct(new_move):
            ...
            # correct player move, game continue
                ...
                print("Player move is: ", new_move)
                chess_table = update_board_sensor_move(new_move)
...
def get_move_coord():
    global best_move, piece_coord, is_castling_move, is_en_passant_move, move_coord, is_promote_move
    
    # castling shinenigans
    if (best_move != "e1g1" and best_move != "e8g8" and best_move != "e1c1" and best_move != "e8c8"): # normal move
        print("best move:", best_move)
        if len(best_move) == 5:
            is_promote_move = True
            move_coord = (piece_coord[best_move[:2]], piece_coord[best_move[2:4]])
        else:
...
def game_loop(data):
    ...
    # Robot's turn
        ...
        if is_capture_move:
            ...
            if capture_coord != -1:
                ...
                serialized_data = json.dumps({"move": first_move, "bool1": False, "bool2": True})
        ...
        # en passant or normal move
            ...
            serialized_data = json.dumps({"move": move_coord, "bool1": False, "bool2": False})
            pub.publish(serialized_data)
    ...
    # Waiting for the robot to move
        ...
        if move_complete:
            if is_capture_move:
                serialized_data = json.dumps({"move": move_coord, "bool1": True, "bool2": False})
                pub.publish(serialized_data)
                is_capture_move = False
                move_complete = False
                ...
                serialized_data = json.dumps({"move": castling_second_move, "bool1": True, "bool2": False})
                pub.publish(serialized_data)
                is_castling_move = False
                move_complete = False
                ...
                serialized_data = json.dumps({"move": en_passant_second_move, "bool1": True, "bool2": False})
                pub.publish(serialized_data)
                is_en_passant_move = False
                move_complete = False
                ...
                piece = best_move[-1]
                capture_coord = get_capture_piece_coord(piece)

                # check if board_sensor returned the piece coordinates correctly
                if capture_coord != -1:
                    promote_second_move = [capture_coord, move_coord[1]]
                    serialized_data = json.dumps({"move": promote_second_move, "bool1": True, "bool2": False})
                    pub.publish(serialized_data)
                    is_promote_move = False
                    move_complete = False
